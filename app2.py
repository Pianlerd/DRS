...
def get_db_connection():
    ...
    try:
        conn = mysql.connector.connect(
            ...
            user="root",
            password="",
            database="project4" # Make sure this matches your database name
        )
        return conn
        ...
        if request.method == "POST":
            ...
            if action == 'add':
                ...
                try: # Wrapped in an inner try-except for specific add errors
                    ...
                    if product_info['store_id'] != op_store_id: # Ensure product belongs to the current store context
                ...
                try:
                    ...
                    # Ensure the order's store_id matches the user's current context
                    if old_order_info['store_id'] != op_store_id:
                    ...
                    # Ensure the new product also belongs to the current store context
                    if new_product_info['store_id'] != op_store_id:
                ...
                try:
                    ...
                    # Ensure the order's store_id matches the user's current context
                    if order_to_delete['store_id'] != op_store_id:
